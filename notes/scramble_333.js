...
var scramble_333 = (function(getNPerm, get8Perm, setNPerm, set8Perm, getNParity, getPruning, Cnk, fact, rn, rndEl) {
    ...
    function initSym() {
        ...
        // console.log("Sym8Move")
        // Sym8Move.forEach(function(a){
        //     console.log(a.join(', '))
        // });

        // console.log("Sym8Mult")
        // Sym8Mult.forEach(function(a){
        //     console.log(a.join(', '))
        // });
    }
    ...
    function $phase1(obj, twist, tsym, flip, fsym, slice, maxl, lm) {
        ...
        for (axis = 0; axis < 18; axis += 3) {
            ...
            for (power = 0; power < 3; ++power) {
                ...
                MoveSetCount += 1
                //debugger;
                console.log("set move", MoveSetCount, obj.move, obj.depth1, maxl, m)
        ...
        for (obj.depth1 = 0; obj.depth1 < obj.sol; ++obj.depth1) {
            ...
            for (obj.urfIdx = 0; obj.urfIdx < 6; ++obj.urfIdx) {
                ...
                for (obj.preIdx = 0; obj.preIdx < 9; obj.preIdx++) {
                    ...
                    if (obj.prun[obj.urfIdx][obj.preIdx] <= obj.depth1) {
                        //debugger;
                        //console.log("calling phase1 from solve", MoveSetCount, obj.move, obj.depth1, obj.urfIdx)
                        var z = $phase1(
                            obj,
                            obj.twist[obj.urfIdx][obj.preIdx] >>> 3,
                            obj.twist[obj.urfIdx][obj.preIdx] & 7,
                            obj.flip[obj.urfIdx][obj.preIdx] >>> 3,
                            obj.flip[obj.urfIdx][obj.preIdx] & 7,
                            obj.slice[obj.urfIdx][obj.preIdx] & 0x1ff,
                            obj.depth1,
                            -1,
                        );
    ...
    function initialize() {
        ...
        initMPermMoveConj();
        epermR2S = null;
        console.log("done with move tables", (+new Date - startTime));
    ...
    return {
        ...
        solve: function(fs){
            ini();
            var s = +new Date;
            var sol = $solution(search, fs);
            console.log("solution", (+new Date - s));
            return sol;
        },
